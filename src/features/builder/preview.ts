...
<body>
  <div id="root"></div>
  <script>
    // ---- Safe import.meta.env shim for generated code ----
    // Apps generated by the AI sometimes use `import.meta.env.PROD`,
    // `import.meta.env.DEV`, `import.meta.env.MODE`, etc. Inside the
    // preview iframe there is no Vite, so `import.meta.env` is undefined
    // and accessing `.PROD` throws "Cannot read properties of undefined".
    // We expose a global fallback that our blob modules read via a small
    // helper injected during transpile (see __LOV_ENV below).
    window.__LOV_ENV = Object.freeze({
      PROD: false,
      DEV: true,
      MODE: 'development',
      SSR: false,
